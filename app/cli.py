...
def analyze(run: str = typer.Option(..., help="Recorded run ID (runs/recorded/<id>)."),
            ...
            refresh_probabilities: bool = typer.Option(False, "--refresh-probabilities",
                                                       help="Ask Jev again instead of using cached forecasts."),
            roles: bool = typer.Option(False, "--roles", help="Recall check: re-ask every forecast with the parties' "
                                       "names replaced by roles; store the deltas (probabilities unchanged).")) -> None:
    ...
    data = build(run, base, refresh=refresh_probabilities, roles=roles)
    ...
    rc = data.get("recall_check")
    if rc:
        typer.echo(f"recall check: {rc['questions']} questions, mean change {rc['mean_abs_change']:.1%}, max "
                   f"{rc['max_abs_change']:.1%}, {len(rc['moved'])} moved more than {rc['threshold']:.0%}; Jev {rc['jev']}")
...
def compare(run: str = typer.Option(..., help="Recorded run ID."), root: str = typer.Option("", help="Run directory."),
    ...
    """Alias of `analyze`."""
    analyze(run=run, root=root, refresh_probabilities=refresh_probabilities, roles=False)
