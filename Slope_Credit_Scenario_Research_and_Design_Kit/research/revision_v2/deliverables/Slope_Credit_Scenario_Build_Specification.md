...
- **Inventory sweep.** Every admissible section is screened once for specific legal matters, settlements, debt agreements, covenants, cash restrictions and matter-related accounting items. The flagged sections form an inventory the agent must account for (covered by accepted findings, or not decision-relevant with a reason) before the packet can be submitted. This replaces prompt rules about coverage.
