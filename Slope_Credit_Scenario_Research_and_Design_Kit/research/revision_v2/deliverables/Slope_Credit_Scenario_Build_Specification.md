...
| `inventory_coverage` | Host, per flagged section, on every inventory item the agent marks covered, duplicate or not decision-relevant | `coverage_supported`, `adds_matter`, `decision_relevance` (Choice) |
...
- **Inventory sweep (a reading list, not an audit).** Every admissible section is screened once for specific legal matters, settlements, debt agreements, covenants, cash restrictions and matter-related accounting items. The flagged sections form an inventory the agent must account for before the packet can be submitted: covered by accepted findings, a duplicate of an item already covered, or not decision-relevant with a reason. Each claim gets one narrow check per flagged section: the findings state the matter's payments, obligations, restrictions, covenants, default terms and earnings items that the section describes, such as an accounting gain (other detail needs no finding); the passage adds nothing to the covered item; or the passage describes no specific matter that could change cash, obligations, underwriting earnings or repayment ability. A clear failure has no free-text override: the agent adds the finding, shows a duplicate, or escalates it citing the failed observation. An escalated item is disputed and goes to the independent reviewer's checklist with the agent's note; the reviewer decides whether it is material, and a material gap that one repair pass does not close makes the run INCOMPLETE_REVIEW. Jev does not prove completeness: finding what is missing across the evidence is the independent reviewer's job (step 6), with one repair pass. This replaces prompt rules about coverage.
