...
Prepared 24 September 2026. Revision 2 replaced the Trinity/A10 implementation direction. **Revision 3 (25 September 2026):** the lead case is ChromaDex at 19 August 2024, whose disputes are live on the decision date; Synergy becomes the secondary case. A live dispute is compiled onto a generic, host-owned post-judgment dispute model: Jev reads each cited passage atomically, code keeps every cash path the evidence permits, and Slope's product, pricing, policy and the connected-bank baseline are reconstructed as closely as public information allows. **Revision 4 (25 September 2026):** the product ends at probabilistic scenario and sensitivity analysis of the lender's dated cash flows for supplied financing terms; it no longer recommends, sizes, tiers or conditions a loan. Jev reads the evidence (present state) and supplies conditional probabilities for narrowly defined future events; code composes them into path probabilities and simulates them against a bootstrapped operating-cash distribution. Stress is a separate view. Where this revision and earlier text differ, this revision governs. **Revision 5 (26 September 2026):** ChromaDex is removed. The lead case is Akoustis Technologies at 20 June 2024 and the second is Charles & Colvard at 18 November 2024, on Slope's reusable line; `Slope_Model_Extensions_Spec.md` governs the cases, the line and the forecast chains, and the ChromaDex sections below are historical.

## 1. The product to build

**Given supplied financing terms and an unusual external event, measure how the researched evidence changes the distribution of the lender's dated cash flows.**

The thesis: **domain-informed probabilistic judgments turn unusual qualitative evidence into a useful, measurable financial signal.** The agent researches evidence that transaction-based underwriting misses. Jev reads that evidence and supplies conditional probabilities for the future events that move cash. Deterministic code composes those probabilities, simulates the borrower's cash against a bootstrapped operating distribution, and carries the result into the loan's collections, collection timing, outstanding exposure, discounted cash flows and capital tied up. Borrower cash is the mechanism; the lender's dated cash flows are the output.

The product stops at scenario and sensitivity analysis that a lending or capital-markets model can consume. It does not recommend, size, tier or condition a loan: those choices bring in risk appetite, pricing objectives, portfolio constraints and other credit signals that the sparse-event research does not establish. A litigation summary is not the deliverable either.

This connects Russell's two topics: unusual borrower events create probabilistic scenarios for an individual loan's collections, and those collections feed portfolio liquidity and capital planning.
...
The analysis takes Slope's product terms as inputs: the financed amount, the repayment schedule and the fee. Public product terms vary by program. Slope’s Amazon program describes fixed repayment schedules with 2–12-month draw terms; its invoice Pay Later materials describe different terms. Select one product configuration; never blend them. [S3–S5]

For this demonstration, **reconstruct Slope's actual product as closely as public information allows**: bill-pay financing, where Slope pays the supplier and the business repays in monthly installments. The lead case supplies $2.0M over three monthly installments at a 3.7% fee. The synthetic connected-bank data is labelled once, at its source, and is otherwise used plainly. The case is reconstructed as if the borrower applied to Slope on the decision date; there is no claim that it did. A borrower's actual existing financing (a bank revolver, a merchant loan) stays a separate existing obligation with its own contract logic.
...
|---|---|---|
| **ChromaDex** | Consumer supplement brand buying from contract manufacturers; live fee award and judgment receivable on the decision date; two federal dockets with filed orders; public financials; public outcome | Lead: live disputes → Jev's conditional probabilities over rule-derived cash paths → distributions of the loan's collections |
...
> As of 19 August 2024, analyse how external events could affect Slope financing of a USD 2.0 million contract-manufacturer invoice for ChromaDex Corporation, repaid in three monthly installments. ChromaDex's public filings describe long-running litigation with Elysium Health, a former customer, in several federal courts.
...
The operator opens **one analysis page** for the run. It carries the causal chain **evidence → Jev judgment → financial mechanism → financial impact** and nothing else. The investigation record stays behind a secondary link; inventories, coverage gates, statuses, reviewer checklists and execution diagnostics never enter the analysis page. There are no approval recommendations, provenance banners or hedging captions.

The page lets Russell move naturally from what the research learned, to what might happen and how likely it is, to what that does to the borrower's cash and to the loan, to which uncertainty matters most:

- **Lender impact:** bank-only versus event-adjusted expected discounted lender cash flows, principal dollar-days (capital tied up), probability of full collection by maturity and uncollected balance, for the supplied terms.
- **Collections timeline:** the contractual schedule, expected cumulative collections and P5–P95 bands, both views.
- **Borrower cash, aligned in time:** median available cash with P5–P95 bands, both views, explaining changes in payment capacity.
- **Ranked sensitivity:** each prospective Jev judgment at 0%, Jev's value and 100%, ranked by its effect on the lender's outcomes, with the borrower-liquidity effect beside it. Unchanged collections are shown plainly when that is the result.
- **Evidence-to-impact drill-down:** selecting a driver shows the source passage, the present-state reading, the exact forecast question with its full distribution and parent assumptions, the cash mechanism and the measured effect.
- **Scenario paths and stress:** the leading joint event paths with their probabilities and effects; all feasible paths, including zero-probability ones, under adverse placement as a separate stress view.
- **Controls:** financed amount (up to the invoice), fee, Delaware exposure, collateral range, operating variability, and a probability override for a selected forecast node with 0%, Jev and 100% actions and reset. Recalculation never calls the agent or Jev.

Slope underwrites on **connected bank data**. The demonstration supplies a synthetic connected-bank feed for the borrower up to the decision date, anchored to the public financial statements and labelled synthetic once, at its source.
...
The lawsuit itself does not supply a generic default-probability increment. It supplies evidence about an economic mechanism. Jev's confidence statistic never becomes a probability of anything. Jev's forecast probabilities are conditional probabilities of narrowly defined future events (labelled model judgment); code composes them into path probabilities and carries them into cash. No fixed default-probability or loss-given-default table is applied.
...
### Supplied terms, probability and stress

The financing terms are inputs: the financed amount (up to the invoice), funding date, repayment schedule, fee and discount rate. No policy engine selects them, and the analysis returns no approval, limit, tier or condition.

**Operating cash** is a calendar-month block bootstrap of the connected-bank feed: the last six complete months, excluding one-off equity proceeds, each category rescaled to its recent three-month level, whole months resampled to preserve relationships between receipts and payments, 512 trajectories from a fixed seed. The bank-only and event-adjusted views use identical draws, so their difference is the effect of the researched events.

**Event paths** come from the dispute model: Jev's conditional probabilities are multiplied along each path; a later dispute with the same counterparty is conditioned on the earlier dispute's outcome for that counterparty. Every structurally feasible joint path is simulated against every operating draw, with weight `joint path probability / 512`, so small-probability paths are kept exactly rather than sampled. Event dates and amounts are drawn inside code-owned windows and ranges, keyed by event so that paths sharing a parent share its draw.

**Stress** is a separate output: every feasible path, including zero-probability ones, under adverse placement. It never determines anything and never acquires probability mass.

**Sensitivity:** each prospective binary judgment at 0%, Jev and 100% (categorical judgments at each option), holding all draws and parameters fixed, ranked by the change in expected discounted lender cash flows, then principal dollar-days, then uncollected balance at maturity. Financial parameters (collateral share, Delaware exposure, operating variability) are reported separately.
...
These fixtures remain as reference arithmetic for the Synergy case; the lead case uses its supplied terms ($2.0M, three monthly installments, 3.7% fee). Default funding to the next business day after the decision date (Slope pays the supplier at once); the operator can change it. Show each structure's APR equivalent beside its fee, and apply Slope's proration of fees on early repayment. Show the minimum residual payment capacity required for each fixture, then apply the event calendar and action-dependent financed purchase plan. No fixture receives an automatic approval. A six-month offer extending into 2025 must include the relevant 2025 settlement timing uncertainty; stopping the legal cash obligations at December would overstate capacity.

### Loan economics and portfolio output

Keep three series separate: contractual amounts owed, conditional cash collected on each path, and probability-weighted collections with P5/P50/P95 bands. The weights are Jev's conditional probabilities composed along each path (labelled model judgment) times the equal-weight operating draws; they sum to one. Retain coherent joint scenarios when aggregating loans.
...
The agent receives the mission and baseline, then decides which gap could change the loan's cash flows. Its loop is: select a material uncertainty and record it as a decision dependency; search eligible evidence for it; read the strongest evidence and conflicts; propose atomic, sourced findings; reconcile contradictions; propose economic effects; run the financial sensitivity; choose the next high-impact dependency; submit a reviewable packet.
...
Jev is a typed, narrow semantic sensor placed where retrieved text becomes a finding, a finding becomes an economic effect, or evidence conflicts. Its value is preventing category errors before they become financial inputs: an allegation treated as a liability, a paid matter treated as a future outflow, a noncash gain treated as cash, the wrong entity, a disputed demand treated as an agreed payment. Jev also **turns a live dispute's evidence into conditional probabilities**, in two stages. The host owns a versioned post-judgment model (`contracts/dispute_model.json`); the agent groups the accepted findings about one obligation and gives its order reference, its nature (a fixed list), the counterparty, the amount figure and any judgment date, all checked; its prose never reaches Jev. **Atomic means one judgment target, not minimal context**: each question is hydrated from the evidence store with the cited passage's surrounding section, source date, parties, procedural position and related passages. (1) **Present-state interpretation**, per cited finding: who pays whom (Choice), the amount's status (Choice), post-judgment interest (Noul), which procedural events the passage establishes (Noul each), and which factors it bears on and at what level (Noul, Score). Established events place the stage; an established waiver or payment constrains the graph. Full distributions are kept. (2) **Conditional forecasting**, per future-event node: settlement before the next step, the amount fixed within the horizon, an appeal within the notice period, a secured stay, the form of security (cash deposit, surety bond, letter of credit), voluntary payment, collection by enforcement, settlement during a secured appeal. Each question names its parent assumptions and a code-set window and receives the relevant hydrated passages and interpretation distributions; Jev returns the probability (Noul) or distribution (Choice). An interpretation reading is never substituted for a forecast: 'intends to appeal' is evidence for 'will appeal within 30 days', not its probability. No hand-written coefficients sit between evidence and probability. Code sets every date, window and amount from cited rules, composes conditional probabilities along paths, and never deletes a feasible path for having low probability. Forecast prompts forbid remembered facts about the parties or later events; residual leakage from Jev's training is a known limit of a historical demonstration.
...
| `dispute_interpretation` | Host, per cited finding of an instantiated dispute (hydrated) | `obligation_direction`, `amount_status` (Choice), `amount_includes_interest`, `event_*`, `bears_on_*` (Noul), `factor_level` (Score) |
| `dispute_forecast` | Host, per future-event node and parent context, when the analysis is built (cached) | `forecast_*` (Noul), `forecast_security_form` (Choice) |

**Jev judges claims; it does not search for what is missing.** A present-state question is used only when (1) there is one judgment target, (2) the supplied evidence (the passage and its hydrated context) contains the answer, and (3) code decides what each answer does. A forecast question is the one forward-looking use: one future event, stated parent assumptions, a code-set window and the relevant evidence; Jev returns a probability and code composes it. Enumeration and completeness belong to the agent and to the independent reviewer.
...
app/
  ...
  disputes/           # dispute model: interpretation, forecast nodes, path composition
  analysis/           # operating bootstrap, weighted simulation, lender metrics, sensitivity, stress
...
| 5b. Probabilistic financial analysis | Synthetic connected-bank baseline (labelled once); supplied financing terms; dispute model with hydrated present-state interpretation and conditional Jev forecasts; path probabilities composed by code, the later same-counterparty dispute conditioned on the earlier; monthly block-bootstrap operating cash (512 draws, fixed seed, identical across views); event dates and amounts drawn in code-owned windows; dated lender cash flows (collections, timing, outstanding exposure, discounted cash flows, principal dollar-days) with bands; 0%/Jev/100% judgment sensitivity ranked by lender effect; separate stress view; one analysis page with evidence-to-impact drill-down and controls; `slope analyze`; recorded ChromaDex run | Russell can follow a real passage into Jev's probabilistic judgment, see that judgment enter the distribution of the loan's dated cash flows, and see which uncertainty matters most; bank-only and event-adjusted views share identical operating draws; no feasible path is deleted for low probability |
| 6. Outcome reveal | After the analysis is locked, what actually happened, shown against the path probabilities in a separate panel | The reveal cannot contaminate the run; it illustrates the analysis and is not a calibration claim |
...
10. Unknown timing, recovery and priority do not become zero, one or a silent default; an explicit 0% or 100% sensitivity is fine.
...
13. Composed path probabilities sum to one, and merged paths preserve total mass.
14. Changing one supplied node probability changes the financial distribution by a hand-computable amount.
15. A zero-probability feasible path remains in the stress set.
16. Without event effects, the bank-only and event-adjusted views match exactly under the same draws.
17. Collateral and credit capacity are released exactly, on the settlement date, once.
...
Critical errors—wrong entity, future leakage, fabricated payment date, demanded amount treated as paid, cash overstated, duplicate obligation, an interpretation reading (an intention or a Score level) substituted for a future-event probability, a Jev probability presented as an observed frequency, a low or zero probability used to remove a feasible stress path—block a publishable demo result.
...
The central demonstration is a traceable chain: **a real passage becomes Jev's conditional probability of a future event; code carries that probability into the distribution of the borrower's cash and the loan's dated collections; the sensitivity shows which judgment matters most to the lender.**

ChromaDex shows two live disputes with the same counterparty whose outcomes move cash both ways inside the loan's life. Synergy's existing financial tests remain the arithmetic foundation. The output is an instrument Russell could recognize in a credit or capital-markets workflow: the lender's dated cash flows, with their uncertainty and its sources.
