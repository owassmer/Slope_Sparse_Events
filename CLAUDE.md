...
## Product judgment
Ask: *does this help the reviewer decide what to fund and understand the cash that comes back?* Keep contractual, conditional, and expected series distinct. Unknown is a typed state, never zero. Deterministic code does the arithmetic. The agent and Jev interpret evidence: the agent thinks broadly, Jev judges narrowly through host-owned judgment profiles (including a live dispute's stage, factors and transitions on the host's dispute model, its distributions weighting the paths), code computes exactly, the host controls everything, and the reviewer sees the causal chain in one investigation graph. A Jev answer never sets an amount, a date or a decision, and never activates a cash adjustment on its own.
