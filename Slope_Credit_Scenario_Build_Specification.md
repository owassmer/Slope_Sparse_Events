...
- **Inventory sweep.** Every admissible section is screened once for specific legal matters, settlements, debt agreements, covenants, cash restrictions and matter-related accounting items. The flagged sections form an inventory the agent must account for (covered by accepted findings, or not decision-relevant with a reason) before the packet can be submitted; a claim that findings cover an item is itself checked. This replaces prompt rules about coverage.
- **Category-error guard.** Every finding that supports an effect gets host-run posture and status checks, with the status question chosen by the effect's mechanism. Code-owned rules require, for example, a required (not claimed or disputed) obligation for a settlement payment effect and a reported-satisfied obligation for a resolved one; a mismatch or ambiguous answer blocks the effect unless the agent overrides it with a reason.
- **Statement support.** Each effect's model consequence and the conclusion are checked against the cited findings and engine results; unsupported claims must be revised or overridden with a reason.
