...
Prepared 24 September 2026. Revision 2 replaced the Trinity/A10 implementation direction. **Revision 3 (25 September 2026):** the lead case is ChromaDex at 19 August 2024, whose disputes are live on the decision date; Synergy becomes the secondary case. A live dispute is placed onto a generic, host-owned post-judgment dispute model whose transitions Jev judges atomically, and Slope's product, pricing, policy and the connected-bank baseline are reconstructed as closely as public information allows.
...
The lawsuit itself does not supply a generic default-probability increment. It supplies evidence about an economic mechanism. Jev’s confidence never becomes a borrower default probability or a loan-loss assumption: default probability comes from the labelled risk tier. On a live dispute's path through the dispute model, Jev's transition distributions weight the paths (labelled model judgment); that changes which cash path is expected, not the borrower's default rate.
...
Keep three series separate: contractual amounts owed, conditional cash collected and probability-weighted expected collections. V1 requires the first two. Expected collections use explicit weights that sum to one and carry their basis: a labelled risk tier from bank data (probability of default), or the products of Jev's transition distributions along a live dispute's paths (labelled once as model judgment; TypeSafe calibrates Jev to frontier-model consensus, not to observed litigation outcomes). The conditional series always sits beside the expected one; retain coherent joint scenarios when aggregating loans.
...
Jev is a typed, narrow semantic sensor placed where retrieved text becomes a finding, a finding becomes an economic effect, or evidence conflicts. Its value is preventing category errors before they become financial inputs: an allegation treated as a liability, a paid matter treated as a future outflow, a noncash gain treated as cash, the wrong entity, a disputed demand treated as an agreed payment. Jev also **judges a live dispute's path through a generic dispute model**. The host owns a versioned post-judgment model (`contracts/dispute_model.json`): stages (liability decided with the amount open, judgment entered, appeal pending, enforcement), generic transitions (the court fixes the amount, the debtor pays, appeals with a bond, settles or resists; the creditor collects or not) and, under each transition, factors in the style of HYPO/CATO (appeal intent, amount finality, the debtor's conduct and liquidity, settlement signals, an appeal waiver). The agent instantiates a dispute onto the model (the cited findings, which side the borrower is on, the counterparty, the documented amount, a judgment date that must appear in the quotes) and never designs branches; it may propose an extension, which is flagged and not used. Jev answers atomic questions: the stage (Choice), which factor a finding bears on (Choice), a graded factor's level (Score) or a factor's presence (Noul), and the next transition given the path so far (Choice). A transition is refined only when it is uncertain and decision-relevant: its findings are routed to its factors, the factors graded, and the transition re-asked with them; factors still unknown become evidence requests, phrased as conditions. Code sets every date and amount from cited rules (Fed. R. App. P. 4, Fed. R. Civ. P. 62 and 69, 28 U.S.C. § 1961, a labelled supersedeas multiple and collateral share, a code-owned ruling window placed early and late), multiplies conditional weights along each path and prunes paths below 1%. Jev never determines amounts, dates or the credit decision, and a Jev answer never activates a cash-flow adjustment on its own. The full lifecycle (pleadings to trial) and calibration against historical dockets are the roadmap.
