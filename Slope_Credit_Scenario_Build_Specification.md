...
| `inventory_coverage` | Host, on every inventory item the agent marks covered (per window, plus each named exclusion), duplicate or not decision-relevant (per flagged section) | `coverage_supported`, `matter_relevance`, `adds_matter`, `decision_relevance` (Choice) |
...
- **Inventory sweep.** Every admissible section is screened once for specific legal matters, settlements, debt agreements, covenants, cash restrictions and matter-related accounting items. The flagged sections form an inventory the agent must account for before the packet can be submitted: covered by accepted findings, a duplicate of an item already covered, or not decision-relevant. Each claim is checked: coverage window by window over every section (at most 2,500 characters each, with the cited findings and their quotes; a finding may state that a matter was paid, closed or superseded; a matter with no payment, obligation, restriction, covenant or earnings effect, such as ownership, governance, procedural or legal-reasoning detail, may instead be named by the agent for its window with a reason, and each named exclusion is checked on its own (`matter_relevance`, no date comparison; a settled matter still needs a finding) and recorded, so nothing is exempted silently), and duplicates and not-decision-relevant claims per flagged section (the passage adds no matter or item to the covered one; it describes no specific matter that could change cash, obligations, underwriting earnings or repayment ability). A clear failure has no free-text override: the agent closes it with a new finding, a verified duplicate, or escalates it, citing the failed observation and what is missing. An escalated item is disputed and stays open for the human reviewer; a disputed legal, settlement, debt, covenant, cash-restriction or matter-accounting item makes the run INCOMPLETE_REVIEW. This replaces prompt rules about coverage.
...
| 4c. Jev at scale | Snapshot sweep inventory with mandatory accounting, category-error guard on effects, statement-support checks, automatic relation checks, screen reliance attribution | No packet can be submitted with an unaccounted inventory item, an unsupported consequence or conclusion, an effect whose supporting findings fail its category rule, or an open reconciliation, unless each exception is resolved or escalated; an escalated matter or cash-moving effect leaves the run INCOMPLETE_REVIEW; the coverage prompt rule is removed; a recorded Synergy run shows these checks at work |
